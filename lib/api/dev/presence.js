...
const LOCAL_STORE_FILE = path.resolve(__dirname, "../../../data/dev-presence.json");
const ADMIN_CODE = process.env.VEL_OS_ADMIN_CODE || process.env.ADMIN_CODE || "admin7945";
const REMOTE_DECK_CODE = process.env.REMOTE_DECK_CODE || "callan123";
...
const REMOTE_SOUND_ACTIVE_MS = 1000 * 60 * 2;
...
function cleanPercent(value = 72) {
  const number = Number.parseInt(value, 10);
  if (!Number.isFinite(number)) return 72;
  return Math.max(0, Math.min(100, number));
}

function cleanSpeed(value = 1) {
  const number = Number.parseFloat(value);
  if (!Number.isFinite(number)) return 1;
  return Math.max(0.45, Math.min(2.5, number));
}

function cleanRemoteAudio(input = {}) {
  const volume = cleanPercent(input.soundboardVolume ?? input.remoteVolume ?? 72);
  const allowed = input.remoteDeckAllowed === true
    || input.remoteDeckAllowed === "true"
    || input.remoteDeckAllowed === "1";
  return {
    remoteDeckAllowed: allowed,
    soundboardVolume: volume,
    remoteSilent: !allowed || volume <= 0
  };
}
...
function getRemoteDeckCode(req, body = {}) {
  return cleanText(req.headers?.["x-vel-remote-code"] || body.remoteDeckCode || getQueryValue(req, "remoteDeckCode"), 64);
}
...
function normalizeFlappyCommand(command = {}) {
  const normalized = normalizeControl(command, "flappy-command");
  if (!normalized) return null;
  const hasSpeed = command.speed !== undefined || command.speedMultiplier !== undefined;
  return {
    ...normalized,
    noCollision: Boolean(command.noCollision),
    speed: hasSpeed ? cleanSpeed(command.speed ?? command.speedMultiplier) : 0
  };
}
...
function normalizeStore(value = {}) {
  ...
  Object.entries(value?.users || {}).forEach(([key, user]) => {
    ...
    if (!person) return;
    const remoteAudio = cleanRemoteAudio(user);
    users[person.userId] = {
      ...person,
  ...
  const flappyCommands = {};
  Object.entries(value?.flappyCommands || value?.flappyClears || {}).forEach(([key, command]) => {
    const normalized = normalizeFlappyCommand({ id: key, ...command });
    if (normalized) flappyCommands[normalized.id || key] = normalized;
  });
  ...
  const remoteSounds = {};
  Object.entries(value?.remoteSounds || {}).forEach(([key, sound]) => {
    const createdAt = Number(sound.createdAt) || Date.now();
    const targetDeviceId = cleanDeviceId({ deviceId: sound.targetDeviceId || sound.deviceId });
    const targetUserId = cleanId(sound.targetUserId || sound.userId, 64);
    if (!targetDeviceId && !targetUserId) return;
    remoteSounds[cleanText(sound.id, 120) || key] = {
      id: cleanText(sound.id, 120) || key,
      targetDeviceId,
      targetUserId,
      soundType: cleanText(sound.soundType, 20) === "file" ? "file" : "generated",
      soundId: cleanText(sound.soundId, 140),
      soundTitle: cleanText(sound.soundTitle, 80) || "Sound",
      remoteVolume: cleanPercent(sound.remoteVolume ?? sound.volumePercent ?? 72),
      fromDeviceId: cleanDeviceId({ deviceId: sound.fromDeviceId }),
      fromUsername: cleanText(sound.fromUsername, 24) || "Remote Deck",
      createdAt
    };
  });

  return { users, bans, kicks, locks, grants, flappyCommands, adminDevices, remoteSounds, announcement: normalizeAnnouncement(value?.announcement) };
}
...
function prunePresence(store) {
  ...
  Object.entries(store.remoteSounds || {}).forEach(([id, sound]) => {
    if (!sound?.createdAt || now - sound.createdAt > REMOTE_SOUND_ACTIVE_MS) {
      delete store.remoteSounds[id];
    }
  });
}
...
async function readStore() {
  ...
  if (!process.env.VERCEL) {
    try {
      ...
      return { ...normalizeStore(JSON.parse(raw)), storage: "file", persistent: true };
    } catch (error) {
      if (error.code !== "ENOENT") console.error("vel.os dev presence file read failed; using memory fallback:", error.message);
    }
  }
...
async function writeStore(store) {
  ...
  if (!process.env.VERCEL) {
    try {
      ...
      await fs.writeFile(LOCAL_STORE_FILE, JSON.stringify(normalized), { mode: 0o600 });
      return { ...normalized, storage: "file", persistent: true };
    } catch (error) {
      console.error("vel.os dev presence file write failed; using memory fallback:", error.message);
    }
  }
...
function consumeFlappyCommands(store, identity) {
  const commands = [];
  Object.entries(store.flappyCommands || {}).forEach(([id, command]) => {
    if (!matchesControl(command, identity)) return;
    commands.push(command);
    delete store.flappyCommands[id];
  });
  return commands
...
function consumeRemoteSounds(store, identity) {
  prunePresence(store);
  const sounds = [];
  Object.entries(store.remoteSounds || {}).forEach(([id, sound]) => {
    const matchesDevice = sound.targetDeviceId && identity.deviceId && sound.targetDeviceId === identity.deviceId;
    const matchesUser = sound.targetUserId && identity.userId && sound.targetUserId === identity.userId;
    if (!matchesDevice && !matchesUser) return;
    sounds.push(sound);
    delete store.remoteSounds[id];
  });
...
function canUseRemoteDeck(req, body = {}, store = {}) {
  if (getAdminCode(req, body) === ADMIN_CODE || getRemoteDeckCode(req, body) === REMOTE_DECK_CODE) return null;
  if (!isAllowedAdminDevice(cleanDeviceId(body), store)) {
    return {
      error: "device_required",
      message: "Enter the Remote Deck code or use a trusted Remote Deck device."
    };
  }
  return null;
}
...
async function handleRemoteList(req, res, body) {
  const store = await readStore();
  const remoteError = canUseRemoteDeck(req, body, store);
  if (remoteError) return sendJson(res, 401, remoteError);
  const saved = await writeStore(store);
  return sendJson(res, 200, {
    ok: true,
    users: serialize(saved).users,
    storage: saved.storage || "memory",
    persistent: Boolean(saved.persistent)
  });
}

async function handleRemoteSound(req, res, body) {
  const store = await readStore();
  const remoteError = canUseRemoteDeck(req, body, store);
  if (remoteError) return sendJson(res, 401, remoteError);
  ...
  if (!target) {
    return sendJson(res, 400, {
      ...
      message: "Pick an online device first."
    });
  }
  const sender = cleanUser(body) || { username: "Remote Deck" };
  const id = `remote-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  const remoteSound = {
    id,
    targetDeviceId: target.deviceId,
    targetUserId: target.userId,
    soundType: cleanText(body.soundType, 20) === "file" ? "file" : "generated",
    soundId: cleanText(body.soundId, 140),
    soundTitle: cleanText(body.soundTitle, 80) || "Sound",
    remoteVolume: cleanPercent(body.remoteVolume ?? body.volumePercent ?? 72),
    fromDeviceId: cleanDeviceId(body),
    fromUsername: sender.username || "Remote Deck",
    createdAt: Date.now()
  };
  store.remoteSounds = { ...(store.remoteSounds || {}), [id]: remoteSound };
  const saved = await writeStore(store);
  broadcastLiveEvent("remote-sound", remoteSound);
  return sendJson(res, 200, {
    ok: true,
    command: "remote-sound",
    target,
    sound: remoteSound,
    storage: saved.storage || "memory",
    persistent: Boolean(saved.persistent)
...
async function handleRemotePoll(req, res, body) {
  const store = await readStore();
  const identity = getIdentity(body);
  const remoteSounds = consumeRemoteSounds(store, identity);
  const saved = await writeStore(store);
  return sendJson(res, 200, {
    ok: true,
    remoteSounds,
...
async function handleControl(req, res, body) {
  ...
  } else if (command === "flappy-remove-pipes") {
    const commandId = `${key}:remove:${Date.now()}`;
    store.flappyCommands[commandId] = normalizeFlappyCommand({
      id: commandId,
      ...target,
      noCollision: true,
      reason: "Disable Flappy pipe collision",
      durationLabel: "One-time Flappy assist",
      speed: 0,
      createdAt: Date.now(),
      createdBy: "Admin"
    });
  } else if (command === "flappy-set-speed") {
    const commandId = `${key}:speed:${Date.now()}`;
    store.flappyCommands[commandId] = normalizeFlappyCommand({
      id: commandId,
      ...target,
      noCollision: false,
      reason: "Set Flappy speed",
      durationLabel: "One-time Flappy speed",
      speed: cleanSpeed(body.speed || body.speedMultiplier || 1),
      createdAt: Date.now(),
      createdBy: "Admin"
    });
...
async function handleAccessCheck(res, body) {
  ...
  const flappyCommands = consumeFlappyCommands(store, identity);
  const flappyControls = flappyCommands.map((flappyCommand) => ({
    noCollision: Boolean(flappyCommand.noCollision),
    speed: Number(flappyCommand.speed) || 0,
    createdAt: flappyCommand.createdAt
  }));
  const remoteSounds = consumeRemoteSounds(store, identity);
  const saved = await writeStore(store);
  ...
  if (lock?.siteLocked) {
    return sendJson(res, 200, {
      ...
      flappyControl: flappyControls[0] || null,
      flappyControls,
      remoteSounds,
      storage: saved.storage,
      persistent: Boolean(saved.persistent)
...
module.exports = async function handler(req, res) {
  try {
    ...
    if (req.method === "POST") {
      ...
      if (action === "remote-list") return handleRemoteList(req, res, body);
      if (action === "remote-sound") return handleRemoteSound(req, res, body);
      if (action === "remote-poll") return handleRemotePoll(req, res, body);
