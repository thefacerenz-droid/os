...
function consumeFlappyCommand(store, identity) {
  const entry = Object.entries(store.flappyCommands || {}).find(([, command]) => matchesControl(command, identity));
  if (!entry) return null;
  delete store.flappyCommands[entry[0]];
  return entry[1];
}
...
async function handleControl(req, res, body) {
  ...
  if (command === "kick") {
    ...
    store.flappyCommands[key] = normalizeFlappyCommand({
      ...(store.flappyCommands[key] || {}),
      id: key,
      ...target,
      noCollision: true,
      reason: "Disable Flappy pipe collision",
      durationLabel: "One-time Flappy assist",
      speed: store.flappyCommands[key]?.speed || 0,
      createdAt: Date.now(),
      createdBy: "Admin"
    });
    ...
    store.flappyCommands[key] = normalizeFlappyCommand({
      ...
      id: key,
      ...target,
      noCollision: Boolean(store.flappyCommands[key]?.noCollision),
...
async function handleAccessCheck(res, body) {
  ...
  const flappyCommand = consumeFlappyCommand(store, identity);
  const remoteSounds = consumeRemoteSounds(store, identity);
  const saved = await writeStore(store);
