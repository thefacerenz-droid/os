...
function normalizeStore(value = {}) {
  ...
  const flappyClears = {};
  Object.entries(value?.flappyClears || {}).forEach(([key, clear]) => {
    const normalized = normalizeControl({ id: key, ...clear }, "flappy-clear");
    if (normalized) flappyClears[normalized.id || key] = normalized;
  });
  ...
  return { users, bans, kicks, locks, grants, flappyClears, adminDevices, remoteSounds, announcement: normalizeAnnouncement(value?.announcement) };
}
...
function consumeFlappyClear(store, identity) {
  const entry = Object.entries(store.flappyClears || {}).find(([, clear]) => matchesControl(clear, identity));
  if (!entry) return null;
  delete store.flappyClears[entry[0]];
  return entry[1];
}
...
async function handleControl(req, res, body) {
  ...
  if (command === "kick") {
    ...
    store.flappyClears[key] = normalizeControl({
      id: key,
      ...target,
      reason: "Remove Flappy tubes",
...
async function handleAccessCheck(res, body) {
  ...
  const flappyClear = consumeFlappyClear(store, identity);
  const remoteSounds = consumeRemoteSounds(store, identity);
  const saved = await writeStore(store);
  ...
  if (lock?.siteLocked) {
    return sendJson(res, 200, {
      ...
      flappyPipeClear: flappyClear ? { createdAt: flappyClear.createdAt } : null,
      remoteSounds,
      storage: saved.storage,
      persistent: Boolean(saved.persistent)
    });
