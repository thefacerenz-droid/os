...
function baseUrl() {
  ...
  if (!configured && process.env.VERCEL) throw paymentError("APP_BASE_URL is missing. Set it to your full https:// website address in Vercel, then redeploy.");
  try {
    const url = new URL(configured || `http://localhost:${process.env.PORT || 3000}`);
    if (!["http:", "https:"].includes(url.protocol) || url.username || url.password || url.pathname !== "/" || url.search || url.hash) throw new Error();
    return url.origin;
  } catch {
    throw paymentError("APP_BASE_URL must be your full website address starting with https://, with no quotes or extra path. Update it in Vercel and redeploy.");
  }
}
...
async function stripe(path, fields) {
  ...
  if (!/^(sk|rk)_(test|live)_\S+$/.test(key)) throw paymentError("STRIPE_SECRET_KEY must contain a Stripe secret key (sk_test_ or sk_live_), not a publishable key. Update it in Vercel and redeploy.");
  ...
  if (!response.ok) {
    // Log identifiers only; upstream messages can contain submitted values.
    console.error("Stripe request rejected", { status: response.status, requestId: response.headers?.get("request-id"), code: data.error?.code, type: data.error?.type, param: data.error?.param });
    if (response.status === 401) throw paymentError("Stripe rejected the secret key. Check STRIPE_SECRET_KEY in Vercel and redeploy.");
    if (response.status === 403) throw paymentError("The Stripe key does not have permission to create or read Checkout Sessions. Check its permissions in Stripe.");
    if (response.status === 429) throw paymentError("Stripe is receiving too many requests. Please wait a moment and try again.", 429);
    throw paymentError("Stripe rejected the checkout request. The site owner can find the Stripe error code and request ID in Vercel function logs.");
  }
  return data;
}
function active(req) {
...
async function handle(req, res, action) {
  try {
    ...
    if (error.publicMessage) return send(res, error.status, { message: error.publicMessage });
    console.error("Billing request failed", { action, type: error.name });
