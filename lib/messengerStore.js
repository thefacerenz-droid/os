...
function createMessengerStore(options = {}) {
  ...
  const storeKey = options.key || STORE_KEY;
  ...
  async function transact(update) {
    ...
    if (mode !== "file") {
      for (let attempt = 0; attempt < 30; attempt += 1) {
        const raw = await command(["GET", storeKey]) || "";
        const { store, result } = update(raw ? JSON.parse(raw) : null);
        if (Number(await command(["EVAL", CAS, "1", storeKey, raw, JSON.stringify(store)])) === 1) return result;
